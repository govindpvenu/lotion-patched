...
const { getThemeCSS } = require("../themes");
...
class TabController {
  constructor({ tabId, windowId, store, initialUrl }) {
    ...
    log.info(
      `TabController initialized for tab: ${this.tabId} in window: ${this.windowId}`,
    );
  }
  ...
  createWebContentsView() {
    ...
    try {
      this.webContentsView.webContents.session.setSpellCheckerLanguages(
        dictionaries,
      );
      log.debug(
        `Tab ${this.tabId}: Spell checker languages set to ${dictionaries}`,
      );
      ...
      this.webContentsView.webContents.session.setSpellCheckerEnabled(
        spellCheckEnabled,
      );
      log.debug(
        `Tab ${this.tabId}: Spell checker enabled: ${spellCheckEnabled}`,
      );
    } catch (err) {
      log.error(
        `Tab ${this.tabId}: Error setting spell checker languages:`,
        err,
      );
    }
  ...
  setupEventListeners() {
    ...
    webContents.on("page-title-updated", (event, title) => {
      ...
      const breadcrumbs = cleanTitle ? [{ id: "page", title: cleanTitle }] : [];
      this.store.dispatch(
        updateTabBreadcrumbs({
          tabId: this.tabId,
          breadcrumbs,
    ...
    webContents.setWindowOpenHandler(({ url }) => {
      ...
      if (isNotionDomain) {
        ...
        log.debug(
          `Tab ${this.tabId}: Allowing new window for Notion URL: ${url}`,
        );
        return { action: "allow" };
      } else {
    ...
    webContents.on("will-navigate", (event, navigationUrl) => {
      ...
      if (!isNotionDomain) {
        ...
        log.debug(
          `Tab ${this.tabId}: Blocked external navigation to ${navigationUrl}`,
        );
      } else {
        log.debug(
          `Tab ${this.tabId}: Allowing internal navigation to ${navigationUrl}`,
        );
      }
    });
  ...
  async removeInsertedCSS(cssKey, label) {
    if (
      !cssKey ||
    ...
    try {
      await this.webContentsView.webContents.removeInsertedCSS(cssKey);
    } catch (err) {
      // Inserted CSS keys are cleared by navigation/reload. A stale key should
      // not prevent the next theme from being injected.
      log.debug(
        `Tab ${this.tabId}: Unable to remove ${label}; it may already be gone`,
        err,
      );
    }
  }
  ...
  /**
   * Inject the selected theme and user's custom CSS.
   */
  async injectCustomCSS(themeName = null) {
    ...
    const currentTheme = themeName || store.get("theme", "default");

    // First, remove any previous theme. This must continue even when the old
    // key is stale after a navigation/reload.
    await this.removeInsertedCSS(this.injectedThemeKey, "theme CSS");
    this.injectedThemeKey = null;

    if (currentTheme !== "default" && currentTheme !== "none") {
      try {
        const { css: themeCSS, source } = getThemeCSS(currentTheme);

        if (themeCSS) {
          this.injectedThemeKey = await webContents.insertCSS(themeCSS);
          log.info(
            `Theme "${currentTheme}" injected for tab ${this.tabId} from ${source}`,
          );
        } else {
          log.warn(`Tab ${this.tabId}: Theme "${currentTheme}" not found`);
        }
      } catch (err) {
        log.error(
          `Tab ${this.tabId}: Error injecting theme "${currentTheme}":`,
          err,
        );
      }
    } else {
    ...
    // Then, inject custom.css so personal overrides stay above bundled themes.
    const customCSSPath = path.join(app.getPath("userData"), "custom.css");
    await this.removeInsertedCSS(this.injectedCSSKey, "custom CSS");
    this.injectedCSSKey = null;

    if (fs.existsSync(customCSSPath)) {
      try {
        const css = fs.readFileSync(customCSSPath, "utf8");
        this.injectedCSSKey = await webContents.insertCSS(css);
  ...
  async loadTheme(themeName) {
    ...
    await this.injectCustomCSS(themeName);
  }
